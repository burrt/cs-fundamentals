...
public final class BubbleSortTest {
    ...
    public void sort_emptyListNoCost_success() {
        // Act
        List<Integer> sorted = this.bubbleSort.sort(new ArrayList<Integer>());

        // Assert
        assertTrue(sorted.isEmpty());
        assertEquals(0, this.bubbleSort.getComparisons());
        assertEquals(0, this.bubbleSort.getSwaps());
    }
    ...
    public void sort_positiveNumbers_success() {
        ...
        // Act
        List<Integer> sorted = this.bubbleSort.sort(unsortedNumbers);

        // Assert
        assertThat(sorted, is(expectedSortedNumbers));
        assertTrue(this.bubbleSort.getComparisons() > 0);
        assertTrue(this.bubbleSort.getSwaps() > 0);
    }
