...
public class InsertionSortTest {
    ...
    public void sort_nullListOfUnsortedNumbers_throws() {
        this.insertionSort.sort(null);
    }

    @Test
    public void sort_emptyListNoCost_success() {
        // Act
        List<Integer> sorted = this.insertionSort.sort(new ArrayList<Integer>());

        // Assert
        assertTrue(sorted.isEmpty());
        assertEquals(0, this.insertionSort.getComparisons());
        assertEquals(0, this.insertionSort.getSwaps());
    }
    ...
    public void sort_positiveNumbers_success() {
        ...
        // Act
        List<Integer> sorted = this.insertionSort.sort(unsortedNumbers);

        // Assert
        assertThat(sorted, is(expectedSortedNumbers));
        assertTrue(this.insertionSort.getComparisons() > 0);
        assertTrue(this.insertionSort.getSwaps() > 0);
    }
